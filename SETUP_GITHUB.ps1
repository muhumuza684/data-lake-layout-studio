...
param(
  ...
  [string]$ProjectPath = "C:\PowerBI\FFLEXAD",
  [switch]$CreateRemote,
  [switch]$Push
)
$ErrorActionPreference = "Stop"
$RepoName = "FFLEXAD-powerbi-visual"
$ExpectedName = "FFLEXAD"
...
if (-not (git diff --cached --quiet)) { git commit -m "Initial FFLEXAD Power BI visual" }
...
if ($CreateRemote) {
  ...
  if ($LASTEXITCODE -ne 0) { gh repo create "$Owner/$RepoName" --public --description "Independent FFLEXAD Power BI custom visual" --source . --remote origin --push }
...
if ($Push) {
  ...
  if ($PSCmdlet.ShouldProcess("origin/main", "Push FFLEXAD")) { git push -u origin main }
}
Write-Host "Prepared FFLEXAD. Remote creation/push are opt-in switches." -ForegroundColor Green
